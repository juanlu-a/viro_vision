...
PR lands here and ships to the internal TestFlight group) and `main` (production; reached by a `staging → main` PR).
...
| **TestFlight** | `.github/workflows/testflight.yml` | push to `staging` or `main` (changes under `app/`), manual from any branch | macOS runner: lint/typecheck/test → prebuild → archive → upload → assign to a TestFlight group and submit to Beta App Review. `staging` → internal group *Equipo ViroVision* (no review); `main` → external group *Testers ViroVision* (public link). See [`dev-build-ios.md`](dev-build-ios.md). |
| **Google Play** | `.github/workflows/android-play.yml` | same triggers as TestFlight; gated on `vars.PLAY_ENABLED` | Linux runner: gates → prebuild → signed `.aab` (upload key) → Google Play Developer API. `staging` → *internal testing*; `main` → *closed testing (alpha)* with opt-in link. See [`android-play.md`](android-play.md). |
